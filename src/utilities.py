...
def generate_annotations(drift_info: pd.DataFrame, dir: str,
    ...
    for log_name in log_names:
        ...
        for drift_id in drift_ids:
            ...
            # Skip the annotation for simple drifts if only complex drift should be annotated
            if cfg.ANNOTATION_TYPE == "only_complex" and drift_type in ["sudden", "gradual"]:
                continue
            ...
            # If annotation should also include information about process changes that belong to complex drift
            if cfg.ANNOTATION_TYPE == "complex_detailed" and drift_type in ['incremental', 'recurring']:
                drift['drift_type'] = drift['change_type']
                for sub_drift_id in  pd.unique(drift["drift_attribute"]):
                    sub_drift = drift.loc[drift["drift_attribute"] == sub_drift_id]
                    sub_drift_type = pd.unique(sub_drift["drift_type"])[0]
                    category_id = get_drift_id(sub_drift_type)
                    if cfg.ANNOTATIONS_ONLY:
                        bbox = get_bbox_as_list_coco_untyped(sub_drift, sub_drift_type)
                    else:
                        bbox = get_bbox_as_list_coco(sub_drift, sub_drift_type)
                    area = get_area(width=bbox[2], height=bbox[3])
                    segmentation = get_segmentation(bbox)
                    log_annotation = {
                        "id": annotation_id,
...
def visualize_batch(path: str, mode: str, seed: int, n_examples=3):
    ...
    plt.savefig(os.path.join(cfg.TRAINED_MODEL_PATH, f"{cfg.BASE_REPRESENTATION}_{mode}_batch.png"),
                bbox_inches="tight")
    plt.close()
...
def visualize_predictions_old(path, mode, model, seed, n_examples=3, threshold=0.50):
    ...
    plt.savefig(os.path.join(cfg.TRAINED_MODEL_PATH, f"{cfg.BASE_REPRESENTATION}_{mode}_predictions.png"),
                bbox_inches="tight")
...
def get_model_config(model_dir: str) -> ExperimentConfig:
    ...
    if cfg.MODEL_SELECTION == "retinanet_spinenet_coco":
        ...
        exp_config.task.model.anchor.aspect_rations = [1]
...
def start_tfr_script(repo_dir: str, data_dir: str, tfr_dir: str, prefix: str):
    ...
    cmd = f"mamba run -n tf_garden_env python -m official.vision.data.create_coco_tf_record --logtostderr \
            --image_dir={data_dir} \
...
def create_winsim_experiment(dir: str) -> str:
    ...
    exp_path = os.path.join(dir, "winsim", f"experiment_{timestamp}_w{cfg.N_WINDOWS}")
