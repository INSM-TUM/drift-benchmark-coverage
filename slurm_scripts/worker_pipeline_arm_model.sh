...
if [[ "$DATASET" == "cdrift" ]]; then
    ...
    for W in "${WINDOWS[@]}"; do
        ...
        if [[ -f "$PRED_FILE" ]]; then
            ...
            python predict.py \
                ...
                --log-dir "$LOG_DIR" \
                --encoding winsim \
                --image-dir "/storage/home/deig/nobackup/cv4cdd/data/cdrift_predictions_arm/winsim_${W}" \
                --n-windows "$W" \
                --base-rep "$BASE_REP" \
    ...
    if [[ -f "$PRED_FILE" ]]; then
        ...
        python predict.py \
            ...
            --log-dir "$LOG_DIR" \
            --encoding winsim \
            --image-dir "/storage/home/deig/nobackup/cv4cdd/data/cdlg_predictions_arm/evaluation/general/threshold_0.5/winsim_images" \
            --n-windows 200 \
            --base-rep "$BASE_REP" \
    ...
    for SUBDIR in "${SUBDIRS[@]}"; do
        ...
        if [[ -f "$PRED_FILE" ]]; then
            ...
            python predict.py \
                ...
                --log-dir "$LOG_DIR" \
                --encoding winsim \
                --image-dir "/storage/home/deig/nobackup/cv4cdd/data/custom_eval_predictions_arm/$SUBDIR" \
                --n-windows 200 \
                --base-rep "$BASE_REP" \
