...
def get_true_changepoints_and_classes(log_info: pd.DataFrame, n_traces: int = None) -> tuple:
    """Get true changepoints as trace indices and true drift types.

    Args:
        log_info (pd.DataFrame): Drift info for event log
        n_traces (int, optional): Total number of traces in the log (needed for fractional trace indices).
    ...
    if "drift_sub_attribute" in log_info.columns:
        drift_ids = log_info["drift_or_noise_id"].unique()
        for drift_id in drift_ids:
            if str(drift_id).startswith("noise"): continue
            
            drift_rows = log_info[log_info["drift_or_noise_id"] == drift_id]
            cp_rows = drift_rows[(drift_rows["drift_sub_attribute"] == "change_trace_index") | 
                                 (drift_rows["drift_sub_attribute"] == "drift_traces_index")]
                                 
            main_type_row = drift_rows[(drift_rows["drift_attribute"] == "drift_type") & 
                                       (drift_rows["drift_sub_attribute"] == "na")]
            main_type = main_type_row["value"].iloc[0] if not main_type_row.empty else "unknown"
            
            if main_type in ["incremental", "recurring"]:
                all_indices = []
                for _, cp_row in cp_rows.iterrows():
                    val_list = utils.special_string_2_list(cp_row["value"])
                    if val_list:
                        all_indices.extend([int(x) for x in val_list])
                if all_indices:
                    all_indices.sort()
                    change_points_trace_idx.append((all_indices[0], all_indices[-1]))
                    drift_types.append(main_type)
            else:
                for _, cp_row in cp_rows.iterrows():
                    sub_attr = cp_row["drift_attribute"]
                    val = cp_row["value"]
                    
                    type_row = drift_rows[(drift_rows["drift_attribute"] == sub_attr) & 
                                          (drift_rows["drift_sub_attribute"] == "change_type")]
                    dtype = type_row["value"].iloc[0] if not type_row.empty else main_type
                    
                    val_list = utils.special_string_2_list(val)
                    if val_list:
                        change_points_trace_idx.append((int(val_list[0]), int(val_list[-1])))
                        drift_types.append(dtype)
    else:
        ...
        for drift_id in drift_ids:
            ...
            if "change_start" in drift_rows["drift_attribute"].values:
                # complex format from previous approach
                starts = drift_rows[drift_rows["drift_attribute"] == "change_start"]["value"].tolist()
                ends = drift_rows[drift_rows["drift_attribute"] == "change_end"]["value"].tolist()
                dtypes = drift_rows[drift_rows["drift_attribute"] == "drift_type"]["value"].tolist()
                for start, end, dtype in zip(starts, ends, dtypes):
                    start_idx = extract_index(start)
                    end_idx = start_idx if str(end).strip() == "N/A" else extract_index(end)
                    change_points_trace_idx.append((start_idx, end_idx))
                    drift_types.append(dtype)
            else:
                col_name = "change_trace_index"
                if "drift_traces_index" in drift_rows["drift_attribute"].values:
                    col_name = "drift_traces_index"
                    
                cp_row = drift_rows[drift_rows["drift_attribute"] == col_name]
                if not cp_row.empty:
                    val = cp_row["value"].iloc[0]
                    val_list = utils.special_string_2_list(val)
                    if val_list:
                        change_points_trace_idx.append((int(val_list[0]), int(val_list[-1])))
                        
                type_row = drift_rows[drift_rows["drift_attribute"] == "drift_type"]
                if not type_row.empty:
                    drift_types.append(type_row["value"].iloc[0])
                elif not drift_rows[drift_rows["drift_attribute"] == "change_type"].empty:
                    drift_types.append(drift_rows[drift_rows["drift_attribute"] == "change_type"]["value"].iloc[0])
                else:
                    drift_types.append("unknown")
...
def evaluate(data_dir: str, model: tf.keras.Model, threshold=0.5):
    ...
    for image_name, image_path in tqdm(images.items(),
        ...
        true_change_points, y_true_category = get_true_changepoints_and_classes(log_info, n_traces=traces_per_log[log_name])
...
def process_image(image_data, model_fn, input_image_size, threshold, log_matching, drift_info, date_info, traces_per_log, category_index, window_info=None, timestamps_per_trace=None):
    ...
    if cfg.ENCODING_TYPE == "winsim":
        bbox_pred = bbox_pred / cfg.TARGETSIZE * cfg.N_WINDOWS
        log_window_info = window_info[log_name]
        pred_change_points = get_changepoints_trace_idx_winsim(bbox_pred, y_pred_category, log_window_info)
    elif cfg.ENCODING_TYPE == "vdd":
        pred_change_points = get_changepoints_trace_idx_vdd(
            bboxes=bbox_pred, y_pred=y_pred_category,
            timestamps_per_trace=timestamps_per_trace[log_name],
            min_date=str_2_date(min_date), max_date=str_2_date(max_date)
        )

    return log_name, true_change_points, pred_change_points, y_true_category, y_pred_category


def evaluate_parallel(data_dir: str, model: tf.keras.Model, threshold=0.5, num_workers=None):
    """Main function for model evaluation.
    Saves evaluation results to output path.
    ...
    Args:
        ...
        threshold (float, optional): Threshold for prediction confidence.
        Defaults to 0.5.
        num_workers (int, optional): Number of CPU cores to use for parallel processing.
    ...
    # Retrieve window and timestamp info based on encoding type
    if cfg.ENCODING_TYPE == "winsim":
        window_info = get_window_info(data_dir)
    elif cfg.ENCODING_TYPE == "vdd":
        timestamps_per_trace = get_first_timestamps_vdd(data_dir)

    # Gather various information from the data directory
    log_matching = get_log_matching(data_dir)
    drift_info = get_drift_info(data_dir)
    date_info = get_date_info(data_dir)
    traces_per_log = get_traces_per_log(data_dir)

    # Decode category index
    category_index, _ = utils.get_ex_decoder()
    eval_results = defaultdict(lambda: defaultdict(dict))
    images = get_image_paths(data_dir)

    # Use ProcessPoolExecutor with specified num_workers (number of CPU cores)
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {
            executor.submit(
                ...
                input_image_size,
                threshold,
                # Ensure to convert any repeated fields in log_matching, drift_info, etc. to lists
                convert_to_list(log_matching),   # Example conversion function
                convert_to_list(drift_info),     # Example conversion function
                convert_to_list(date_info),       # Example conversion function
                convert_to_list(traces_per_log),  # Example conversion function
                convert_to_list(category_index),   # Example conversion function
                window_info if cfg.ENCODING_TYPE == "winsim" else None,
                timestamps_per_trace if cfg.ENCODING_TYPE == "vdd" else None
            ): (image_name, image_path)
            for image_name, image_path in images.items()
        }

        # Process results as they complete
        for future in tqdm(as_completed(futures), total=len(futures), desc="Evaluating model", unit="images"):
            try:
                log_name, true_change_points, pred_change_points, y_true_category, y_pred_category = future.result()
...
def convert_to_list(obj):
    """Convert repeated fields of an object to a list if applicable."""
    
    # Replace with actual types based on your project
    if isinstance(obj, ActualLogMatchingClass):  # Use the actual class for log_matching
        return list(obj.field_name)  # Replace field_name with the actual repeated field name
    elif isinstance(obj, ActualDriftInfoClass):  # Use the actual class for drift_info
        return list(obj.another_field_name)  # Replace another_field_name with the actual name
    elif isinstance(obj, ActualDateInfoClass):  # Use the actual class for date_info
        return list(obj.dates)  # Replace with actual field
    elif isinstance(obj, ActualTraceLogClass):  # Use the actual class for traces_per_log
        return list(obj.traces)  # Replace with actual field
    elif isinstance(obj, ActualCategoryIndexClass):  # Use the actual class for category_index
        return list(obj.categories)  # Replace with actual field
    
    # If the object does not need conversion, return it as-is
    return obj
...
def get_log_matching(data_dir: str) -> pd.DataFrame: 
    ...
    log_matching = pd.read_csv(log_matching_path, index_col=0, sep=None, engine="python")
...
def get_traces_per_log(data_dir: str) -> Union[list, dict]:
    ...
    if os.path.isfile(number_of_traces_path):
        file = open_file(number_of_traces_path)
        return json.load(file)
    
    # Fallback: Count traces manually by parsing .xes files in data_dir
    traces_per_log = {}
        ...
        if filename.endswith(".xes"):
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                for line in f:
                    if "<trace" in line:
                        count += 1
            traces_per_log[filename] = count
            
    if not traces_per_log:
        raise AssertionError(f"No number of traces file found in {data_dir} and no .xes files to fallback on.")
        
    return traces_per_log
...
def get_drift_info(data_dir: str) -> pd.DataFrame:
    ...
    return pd.read_csv(drift_info_path, sep=None, engine="python")
...
def create_evaluation_dir(path: str, data_dir: str) -> str:
    ...
    if check.startswith('w') and check[1:].isdigit():
        evaluation_folder_name = f"evaluation_{check}"
    else:
