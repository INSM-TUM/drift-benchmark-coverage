...
def getTP_FP(detected: List[int], known: List[int], lag: int,
    ...
    if count_duplicate_detections:
        ...
        true_positive_candidates = [d for d in detected if any(
            (max(abs(d[0] - k[0]), abs(d[1] - k[1])) <= lag) for k in known)]
...
def assign_changepoints(detected_changepoints: List[Tuple[int, int]], 
    ...
    def buildProb_NoObjective(sense):
        ...
        for dp in detected_changepoints:
            for ap in actual_changepoints:
                ds = dp[0] if isinstance(dp, (tuple, list)) else dp
                de = dp[1] if isinstance(dp, (tuple, list)) else dp
                as_ = ap[0] if isinstance(ap, (tuple, list)) else ap
                ae = ap[1] if isinstance(ap, (tuple, list)) else ap
                
                dist = max(0, as_ - de, ds - ae)
                
                prob += (x[dp, ap] * dist <= lag_window,
                            f"Distance_Within_Lag_Window_{ds}_{as_}_{unique_count}"
                        )
                unique_count += 1
        return prob, x
    ...
    def get_dist(dp, ap):
        ...
        as_ = ap[0] if isinstance(ap, (tuple, list)) else ap
        ae = ap[1] if isinstance(ap, (tuple, list)) else ap
        return max(0, as_ - de, ds - ae)

    prob2 += (
        lpSum(
            # Minimize the squared distance between assigned intervals
            prob2_vars[dp, ap] * pow(get_dist(dp, ap), 2)
            for dp in detected_changepoints for ap in actual_changepoints
        ),
        "Squared_Distances_Intervals"
    )
