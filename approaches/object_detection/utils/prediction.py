...
def winsim_pipeline(log_dir: str, output_path: str, n_windows: int):
    ...
    if os.path.exists(window_info_path):
        with open(window_info_path, "r", encoding="utf-8") as file:
    ...
    for name, path in tqdm(log_files.items(), desc="Preprocessing Event Logs",
                           unit="Event Log"):
        real_name = name.split(".")[0]
        img_path = os.path.join(output_path, f"{real_name}.jpg")
        if os.path.exists(img_path) and real_name in window_info:
            continue
        ...
        if getattr(pp.cfg, 'BASE_REPRESENTATION', 'dfg') == 'arm':
            windowed_matrices, _, window_information, _ = \
                pp.log_to_windowed_arm(filtered_log, n_windows)
