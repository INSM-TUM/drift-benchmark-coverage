...
def main_cp_only_external(full_path, LAG):
    ...
    log_data = pd.read_csv("/storage/home/deig/nobackup/ARM-based-CV4CDD_backup/data/input_cdrift/log_size_info.csv")
...
#        log_size = 2999
    ...
    for index, row in df.iterrows():
        ...
        # get actual change point infos
        actual_info_rel = ast.literal_eval(log_data.loc[log_data['log_name'] == row['Unnamed: 0'], "change_points"].values[0])

        # get log size
#        breakpoint()
        log_size = log_data.loc[log_data['log_name'] == row['Unnamed: 0'], "n_traces"].values[0] 
...
def process_lag(args):
    folder, LAG, base_path = args
    path = os.path.join(base_path, folder)
#    breakpoint()
    print(os.path.basename(path))
    print(os.path.basename(path).startswith("winsim"))
    if os.path.basename(path).startswith("winsim"):
...
#    report = main_cp_only(full_path, LAG)
    ...
    if folder.split("_")[-1].isdigit():
        report["windows"] = int(folder.split("_")[-1])    
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate SCDD 4D on external data")
    parser.add_argument("--base-path", dest="base_path", type=str, 
                        default="/storage/home/deig/nobackup/cv4cdd/data/cdrift_predictions",
                        help="Base path containing the prediction folders (e.g. winsim_60, ...)")
    parser.add_argument("--num-cores", dest="num_cores", type=int, default=45,
                        help="Number of CPU cores to use for multiprocessing")
    cmd_args = parser.parse_args()

    base_path_main = cmd_args.base_path
    #folders = ["winsim_60", "winsim_70", "winsim_75", "winsim_80", "winsim_90", "winsim_100", "winsim_110", "winsim_120", "winsim_125", "winsim_130", "winsim_140", "winsim_150", "winsim_160", "winsim_170", "winsim_175", "winsim_180", "winsim_190", "winsim_200"]
    folders = ["winsim_60", "winsim_70", "winsim_80", "winsim_90", "winsim_100", "winsim_110", "winsim_120", "winsim_130", "winsim_140", "winsim_150", "winsim_160", "winsim_170", "winsim_180", "winsim_190", "winsim_200"]

    all_args = [(folder, LAG, base_path_main) for folder in folders for LAG in cfg.RELATIVE_LAG]
    ...
    report_all.to_csv(os.path.join(base_path_main, f'prediction_results_evaluated_all_windows.csv'), index=True)
