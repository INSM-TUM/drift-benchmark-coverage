...
def run_cdlg_aggregation():
    base_dir = "/storage/home/deig/nobackup/cv4cdd/data/cdlg_predictions_arm_model_arm"
    methods = ["arm", "dfg", "dfg_io"]
    latencies = [0.01, 0.025, 0.05]

    noise_info_path = os.path.join("/storage/home/deig/nobackup/cv4cdd/data/input_cdlg/test/", "noise_summary.csv")
    noise_dict = {}
    if os.path.exists(noise_info_path):
        ...
        noise_df_clean = noise_df.groupby("log_name")["noise"].max().to_dict()
    ...
    for method in methods:
        csv_path = os.path.join(base_dir, "combined_results.csv")
